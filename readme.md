...
- **Metrics & analytics** — Aggregated engagement metrics per session and across a series: total registrations, attendees, unique account domains, and warm-account influence tracking. Series-level metrics are accessible via a metrics icon in the series header, which opens a side drawer showing the full breakdown.
- **Public series landing page** — Series owners can publish an anonymous-facing landing page (`/public/series/<id>`) displaying the series title, details, and session list with registration links. Visibility is owner-controlled via a globe icon in the series header action bar (defaults to private); clicking the icon opens a popover with the on/off toggle and a direct link when public. The page supports a configurable banner image (`Series.ImageUrl`) with a built-in SVG fallback. Each session row is expandable to reveal its rich-text description before the Register button, so registrants can read about a session before committing. Sessions with a recording URL also surface a Watch Recording link on the public page.
