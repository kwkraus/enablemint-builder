...
- **Metrics & analytics** — Aggregated engagement metrics per session and across a series: total registrations, attendees, unique account domains, and warm-account influence tracking. Metrics are accessible via a header icon on the series detail page and display in a right-side overlay drawer.
- **Public series landing page** — Series owners can publish an anonymous-facing landing page (`/public/series/<id>`) displaying the series title, details, and session list with registration and recording links. Visibility is owner-controlled via a globe icon in the series detail header (shows public/private state and opens a popover with full controls); defaults to private. The page supports a configurable banner image (`Series.ImageUrl`) with a built-in SVG fallback. Each session row is expandable to reveal its rich-text description before the Register button, so registrants can read about a session before committing. The layout is compact with session actions rendered as inline text links.
